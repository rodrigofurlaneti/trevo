...
function formataDataeHora(campo, evt) {
    xPos = PosicaoCursor(campo); //dd/MM/yyyy 
    evt = getEvent(evt);
    var tecla = getKeyCode(evt);
    if (!teclaValida(tecla)) return;
    vr = campo.value = filtraNumeros(filtraCampo(campo));
...
function formataHoraMinuto(campo, evt) {
    xPos = PosicaoCursor(campo); //HH:MM 
    evt = getEvent(evt);
    var tecla = getKeyCode(evt);
    if (!teclaValida(tecla)) return;
    vr = campo.value = filtraNumeros(filtraCampo(campo));
...
function formataTelefone(campo, evt) {
    //(00) 0000-00000
    //if (campo.value.match(/\d/g).length > 11)
    //    campo.value = 
    var xPos = PosicaoCursor(campo);
    evt = getEvent(evt);
    var tecla = getKeyCode(evt);
    ...
    else if (tam >= 6) campo.value = "(" + vr.substr(0, 2) + ") " + vr.substr(2, 4) + "-" + vr.substr(6, 5);
    //( // 
...
function SomenteValor(e) {
    var tecla = (window.event) ? event.keyCode : e.which;
    if ((tecla > 47 && tecla < 58)) return true;
    else {
        if (tecla == 8 || tecla == 9 || tecla == 0 || tecla == 44) return true;
        else return false;
    }
}
...
function FormatarValor(obj) {
    ...
    if (obj.value != '') {
        ...
        if (temVirgula == true) {
            ...
            if (obj.value.length >= c + 4) {
                obj.value = obj.value.substring(0, c + 3);
            }
